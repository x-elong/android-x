...
import com.example.eletronicengineer.db.My.UrgentPeoplesEntity
...
//        mMultiStyleItemList.add(item)
        ...
        val result = NetworkAdapter().getDataUrgentPeople().subscribe({
            val urgentPeoples = it.message
                for (j in urgentPeoples){
                    val item = MultiStyleItem(MultiStyleItem.Options.SHIFT_INPUT,j.urgentPeopleName,false)
                    item.jumpListener = View.OnClickListener {
                        val bundle = Bundle()
                        bundle.putString("urgentPeople", GsonBuilder().create().toJson(j,UrgentPeoplesEntity::class.java))
