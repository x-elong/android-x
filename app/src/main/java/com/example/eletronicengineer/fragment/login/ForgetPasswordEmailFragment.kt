...
import kotlinx.android.synthetic.main.fragment_forget_password_email.view.*
...
class ForgetPasswordEmailFragment :Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_forget_password_email,container,false)
        initFragment(view)
        return view
    }
    ...
    private fun initFragment(v: View) {
        ...
        v.tv_forget_password_get_check.setOnClickListener {
            ...
            if (phoneNum.isBlank()) {
                val toast = Toast.makeText(context, "邮箱不能为空！", Toast.LENGTH_SHORT)
                toast.setGravity(Gravity.CENTER, 0, 0)
                toast.show()
            }else {
                if (v.tv_forget_password_get_check.text == "重新获取验证码" || v.tv_forget_password_get_check.text == "获取验证码") {
                    val result = retrievePasswordSendMailCode(phoneNum).subscribeOn(
                        Schedulers.io()
                    ).observeOn(AndroidSchedulers.mainThread()).subscribe(
                        {
                            ToastHelper.mToast(v.context, it.message)
                        }, {
        ...
        v.tv_forget_password_next.setOnClickListener {
            ...
            if (phoneNum.isBlank()) {
                ...
                loadingDialog.show()
                val result = retrievePasswordValidMailCode(phoneNum, registerPassword)
