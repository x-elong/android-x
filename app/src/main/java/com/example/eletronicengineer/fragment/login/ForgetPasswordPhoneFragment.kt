...
import kotlinx.android.synthetic.main.fragment_forget_password_phone.view.*
...
class ForgetPasswordPhoneFragment: Fragment()  {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_forget_password_phone,container,false)
        initFragment(view)
        return view
    }
        ...
        v.tv_forget_password_get_check.setOnClickListener {
            ...
            if (phoneNum.isBlank()) {
                ...
                if (v.tv_forget_password_get_check.text == "重新获取验证码" || v.tv_forget_password_get_check.text == "获取验证码") {
                    val result = sendMobile(phoneNum).subscribeOn(
                        Schedulers.io()
                    ).observeOn(AndroidSchedulers.mainThread()).subscribe(
                        {
                            ToastHelper.mToast(v.context, it.desc)
                        }, {
