...
class EngineeringAppliancesFragment : Fragment() {
    companion object{
        fun newInstance(args:Bundle): EngineeringAppliancesFragment
        {
            val fragment= EngineeringAppliancesFragment()
    ...
    // function:
    fun switchAdapterCheck(adapterGenerate: AdapterGenerate, type: Int,view: View): EngineeringAppLiancesAdapter {
        lateinit var adapter: EngineeringAppLiancesAdapter
        when (type) {
            //自查自检
            Constants.Subitem_TYPE.SELF_EXAMINATION.ordinal->{
//                view.tv_title_title1_checkbox.text = "工器具"
//                adapter = adapterGenerate.Professional_EngineeringAppliances()
                view.tv_title_title1_checkbox.text = "材料"
                adapter = adapterGenerate.ProfessionalMaterial()
...
//                adapter = adapterGenerate.Professional_WorkingPlan()
//            }
        }
        return adapter
    }
